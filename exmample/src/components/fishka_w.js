...
let ki=0;
let fishka_y=[];
export default class Fishka_w extends Component {
        ...
        if(this.props.tid===this.props.id && this.state.inis ){
            return (
                <View style={{position:'absolute'}}>
                    ...
                    <View style={{position:'absolute'}}><Animated.Image source={require('./CS1-W.png')} style={[this.state.pan.getLayout(),{width:23,height:23, position: 'absolute',}]}{...this.panResponder.panHandlers}/></View>
                </View>
            ...
            return (
                ...
                <Animated.Image source={require('./CS1-W.png')} style={[this.state.pan.getLayout(),{width:23,height:23, position: 'absolute',}]}{...this.panResponder.panHandlers} />
            )}
    }
    ...
    _yellow (my,mx,id){
        ...
        if(dice_data1[1].index.length===4){
            dice_data1[1].all-=id;
            for(let i=dice_data1[1].index.length-1;i>=0;i--)
            {
                if(dice_data1[1].index[i]>dice_data1[1].all) dice_data1[1].index[i]=-1;
            }
            if(dice_data1[1].all===0) dice_data1[0]=!dice_data1[0];}
        else
        {
            dice_data1[1].all-=id;
            if( dice_data1[1].index[0]===id) dice_data1[1].index[0]=-1;
            if( dice_data1[1].index[1]===id) dice_data1[1].index[1]=-1;
            dice_data1[1].index[2]=-1;
            if(dice_data1[1].all===0) dice_data1[0]=!dice_data1[0];
        }
        ...
        if(dice_data1[1].all==0) {this.props._okay();dice_data1[1]={};}
    ...
    pushYellow(obj){
        ...
        if(arr.length===4) {
            ...
            for(let i=0;i<app.length;i++)
            {
                if(app[i].ok)
                {
                    let m= app[i].index + this.state.i;


                    if(data[m][0].fishka!==1)
                    {
                        for(let j=0;j<14;j++)
                        {
                            if (j === 0 && data[m][j].fishka === 0) {
                                ...
                                if (data[m][j].fishka === 2 && data[m][j + 1].fishka === 0) {
